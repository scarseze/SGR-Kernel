...
from core.types import PolicyStatus  # Use Platform Types
...
class PolicyEngine:
    ...
    def check(self, skill: BaseSkill, input_data: Any, state: ExecutionState) -> PolicyDecision:
        """
        ...
        try:
            ...
            if meta.risk_level == "high":
                if not perms["allow_high_risk"]:
                    return PolicyDecision(
                        status=PolicyStatus.DENY, reason=f"Role '{user_role}' cannot execute High Risk skills."
                    )

                return PolicyDecision(
                    status=PolicyStatus.REQUIRE_APPROVAL, reason=f"Skill '{skill.name}' is classified as High Risk."
                )
            ...
            if meta.cost_class == "expensive":
                ...
                return PolicyDecision(
                    status=PolicyStatus.REQUIRE_APPROVAL, reason=f"Skill '{skill.name}' is classified as Expensive."
                )

        except Exception as e:
            return PolicyDecision(status=PolicyStatus.REQUIRE_APPROVAL, reason=f"Metadata verification failed: {e}")
        ...
        if hasattr(skill, "is_sensitive"):
            try:
                ...
                return PolicyDecision(status=PolicyStatus.DENY, reason=f"Error during sensitivity check: {e}")
