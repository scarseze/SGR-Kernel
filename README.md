...
* **Распределенная наблюдаемость (Jaeger UI)**: Интеграция с OpenTelemetry и Jaeger для трейсинга асинхронных графов.
  * *Для локального просмотра: запустите `docker-compose up` и откройте `http://localhost:16686`.*
  * ![Jaeger Dashboard](https://via.placeholder.com/800x400?text=Jaeger+Dashboard:+Router+->+KnowledgeAgent+->+WriterAgent)
* **Долговременная память**: Автоматический механизм затухания (Time Decay) и LLM-разрешение конфликтов в базе знаний (VectorDB).
* **Стейт-менеджмент**: Точки сохранения (Checkpoints) и команда `/rollback` в интерфейсах (CLI / WebUI / Telegram) для отката состояния.
* **Human-in-the-Loop**: Автоматическая пауза выполнения (`PAUSED_APPROVAL`) и эскалация человеку, когда ИИ-критик находит ошибки.
* **Формальная верификация**: Все критические инварианты (Atomic Visibility, RollbackSafety) верифицированы с помощью TLA+.
...
*Тестирование проводилось с помощью Locust на 8-core AWS EC2 (c5.2xlarge). Накладные расходы ядра не включают время инференса LLM.*

**Задержка оркестрации ядра: < 500мс**

* ✅ **100 параллельных агентов** @ p95 задержка < 500мс
* ✅ **1000 параллельных агентов** @ p95 задержка < 2с (Точка деградации)
* ✅ **Потребление памяти**: ~50MB RAM на активную сессию (масштабируется линейно, стейт выносится в Redis).
