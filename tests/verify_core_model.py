...
from core.runtime import CoreEngine as RuntimeKernel
...
class MockFileRead(Skill):
    @property
    def name(self) -> str:
        return "fs_read"  # Matches default mock
...
async def verify_core_model():
    ...
    try:
        ...
        async def mock_generate_plan(user_input):
            return PlanIR(
                steps=[StepNode(id="step_1", skill_name="fs_read", inputs_template={"path": "test.txt"})], edges=[]
            )
