...
class PolicyMiddleware(SkillMiddleware):
    ...
    async def before_execute(self, ctx: SkillExecutionContext):
        ...
        ctx.trace.policy_events.append(
            PolicyEvent(
                step_id=ctx.step_id,
...
class ApprovalMiddleware(SkillMiddleware):
    ...
    async def before_execute(self, ctx: SkillExecutionContext):
        ...
        ctx.trace.approval_events.append(
            ApprovalEvent(
                step_id=ctx.step_id, skill_name=ctx.skill_name, approved=is_approved, reason="Policy/High Risk"
            )
        )
