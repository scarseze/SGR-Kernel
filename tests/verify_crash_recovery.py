...
class TestCrashRecovery(unittest.IsolatedAsyncioTestCase):
    async def test_crash_and_resume(self):
        ...
        async def mock_plan(user_input):
            ...
            s1 = StepNode(
                id="s1",
                skill_name="step1_writer",
                inputs_template={"filename": "step1.txt", "content": "done"},
                required_capabilities=["IO"],
            )
            s2 = StepNode(
                id="s2",
                skill_name="step2_crasher",
                inputs_template={"filename": "step2.txt", "content": "CRASH_NOW"},
                required_capabilities=["IO"],
            )
