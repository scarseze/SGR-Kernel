...
class Router:
    ...
    def route_step(self, plan_confidence: float, validation_result: ValidationResult) -> RoutingDecision:
        """
        ...
        if not validation_result.valid:
            return RoutingDecision(
                action=RoutingAction.REPLAN, reason=f"Validation failed: {validation_result.errors}", confidence=0.0
            )

        # 2. Check confidence thresholds
        if plan_confidence >= self.config.min_confidence_proceed:
            return RoutingDecision(
                action=RoutingAction.PROCEED, reason="High confidence plan.", confidence=plan_confidence
            )
