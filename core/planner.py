...
class PlanStep(BaseModel):
    ...
    depends_on: List[str] = Field(
        default_factory=list, description="List of step_ids that must complete before this one"
    )
...
class ExecutionPlan(BaseModel):
    ...
    direct_response: Optional[str] = Field(
        None, description="If no skills are needed (chitchat), providing the response here."
    )
...
class Planner:
    ...
    async def create_plan(
        self, user_request: str, skills_desc: str, history: str
    ...
    async def repair_plan(
        self, original_plan: ExecutionPlan, failed_step_id: str, error: str, history: str
    ) -> ExecutionPlan:
        """
        ...
        try:
            plan, _ = await self.llm.generate_structured(
                system_prompt=system_prompt, user_prompt=user_prompt, response_model=ExecutionPlan
            )
            return plan
        except Exception:
            # Fallback
