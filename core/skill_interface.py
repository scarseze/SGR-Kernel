...
class SkillRuntimeAdapter:
    ...
    async def execute_skill(self, skill_name: str, ctx: SkillContext) -> SkillResult:
        """
        ...
        try:
            ...
            if "ctx" in sig.parameters:
                ...
                # EXECUTE (Legacy v1, requires params and state)
                inp = getattr(skill, "input_schema", dict)()
                if hasattr(inp, "__fields__"):
                    try:
                        inp = inp(**ctx.config)
                    except Exception:
                        pass
                
                result = await skill.execute(inp, ctx.execution_state)
