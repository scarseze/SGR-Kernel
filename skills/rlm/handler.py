...
from skills.rlm.proxy_server import app
...
"""
    ...
    async def process(self, query: str, context: str, state: ExecutionState) -> str:
        history = []
...
class RLMSkill(BaseSkill[RLMInput]):
    name: str = "rlm_skill"
    ...
    async def execute(self, ctx: SkillExecutionContext) -> str:
        params = RLMInput(**ctx.params)
        state = ctx.state
        ...
        if not self._is_proxy_running():
            ...
            t = threading.Thread(
                target=uvicorn.run,
                args=(app,),
                kwargs={"host": "0.0.0.0", "port": 8090, "log_level": "error"},
                daemon=True,
            )
            t.start()
