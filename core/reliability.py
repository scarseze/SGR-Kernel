...
class ReliabilityStrategy(ABC):
    ...
    @abstractmethod
    def decide(
        self, policy: RetryPolicy, failure_type: SemanticFailureType, attempts: int, tier_used: str
    ) -> RecoveryAction:
        pass
...
class RFCv2ReliabilityStrategy(ReliabilityStrategy):
    ...
    def decide(
        self, policy: RetryPolicy, failure_type: SemanticFailureType, attempts: int, tier_used: str = "fast"
    ) -> RecoveryAction:
...
class ReliabilityEngine:
    ...
    def __init__(self, strategy: ReliabilityStrategy | None = None):
