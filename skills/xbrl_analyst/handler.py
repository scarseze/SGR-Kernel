...
class XBRLAnalystSkill(BaseSkill[BaseModel]):
    ...
    async def execute(self, params: XBRLInput, state: ExecutionState) -> Any:
        if etree is None:
        ...
        if not os.path.exists(file_path):
            potential_paths = [os.path.join(os.getcwd(), "data", file_path), os.path.join(os.getcwd(), file_path)]
    ...
    def _parse_xbrl_logic(self, file_path):
        ...
        for child in root:
            ...
            facts.append({"concept": concept, "value": child.text, "unit": child.get("unitRef")})
    ...
    def _calculate_metrics_logic(self, facts):
        ...
        if assets == 0:
            assets = get_val("AssetsTotal")  # Try alias
