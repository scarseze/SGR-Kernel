...
from core.trace import RAGQueryTrace, RequestTrace, StepTrace, TraceManager
...
class TestTraceMetrics(unittest.TestCase):
    ...
    def test_metrics_formatting_logic(self):
        ...
        qa = RAGQueryTrace(query="test query", latency_ms=100.0, found_docs=5, used_docs=2, sources=["doc1", "doc2"])
        step.rag_queries.append(qa)
        trace.steps.append(step)

        # Simulation of bot logic
        _txt = f"🆔 `{trace.request_id[:8]}`\n"
        rag_queries = []
