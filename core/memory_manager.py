...
class MemoryManager:
    ...
    async def load_context(self, user_id: str, state: ExecutionState):
        """
        ...
        try:
            ...
            logger.info(f"Memory loaded (recent_count={len(history)}, has_summary={bool(summary_text)})")
    ...
    async def augment_with_semantic_search(self, query: str, state: ExecutionState, limit: int = 5):
        """
        ...
        try:
            ...
            if relevant_history:
                existing_contents = {m.content for m in state.history}
                found_count = 0
