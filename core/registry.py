...
class CapabilityRegistry:
    ...
    @classmethod
    def check_capabilities(cls, skill_name: str, granted_capabilities: Set[str]) -> bool:
        """
...
CapabilityRegistry.register_capability(
    Capability(name="write_fs", description="Write to file system", risk_level="high")
)
...
CapabilityRegistry.register_capability(
    Capability(name="llm_heavy", description="LLM heavy reasoning", risk_level="medium")
)
CapabilityRegistry.register_capability(
    Capability(name="external_api", description="Call external APIs", risk_level="medium")
)
