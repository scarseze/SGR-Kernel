...
class ResearchSubAgent(BaseSkill[BaseModel]):
    """
    ...
    async def execute(self, params: ResearchInput, state: ExecutionState) -> Any:
        ...
        rag_docs_count = 0
        if self.rag:
            print(f"[{self.name}] 📚 Checking internal knowledge base...")
            try:
                ...
                if context:
                    ...
                    print(f"[{self.name}] ✅ Found {rag_docs_count} relevant documents.")
            except Exception as e:
                print(f"[{self.name}] ⚠️ RAG check failed: {e}")
        ...
        report_str = f"### 🧠 Deep Research Report\n\n{report}\n\n*(Generated by Sub-Agent)*"
