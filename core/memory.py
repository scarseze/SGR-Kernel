...
class PersistentMemory:
    def __init__(
        ...
        self.collection_name = os.getenv("QDRANT_COLLECTION", "episodic_memory")
    ...
    async def _index_message(self, user_id: str, role: str, content: str, timestamp: datetime):
        ...
        await self.vector_store.upsert(
            collection=self.collection_name, points=[PointStruct(id=point_id, vector=embedding, payload=payload)]
        )
    ...
    async def _fetch_history(self, session, user_id, limit):
        stmt = select(History).where(History.user_id == user_id).order_by(desc(History.timestamp)).limit(limit)

        result = await session.execute(stmt)
        msgs = result.scalars().all()

        return [Message(role=m.role, content=m.content, timestamp=m.timestamp) for m in reversed(msgs)]
    ...
    async def search_history(self, query: str, limit: int = 5, score_threshold: float = 0.5) -> List[Message]:
        ...
        try:
            ...
            results = await self.vector_store.search(
                collection=self.collection_name, vector=vector, limit=limit, score_threshold=score_threshold
            )
